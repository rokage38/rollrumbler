...
export class Prediction {
  ...
  step(snaps, myId, inp, dt, rtt, now) {
    ...
    if (!me || !me[5] || latest.ph !== PHASE.PLAY) { this.state = null; return null; }
    const lag = Math.min(0.4, (rtt / 2 + (now - latest.rt)) / 1000);
    const tx = me[1] + me[3] * lag, tz = me[2] + me[4] * lag;
    let pr = this.state;
    if (!pr) pr = this.state = { x: tx, z: tz, vx: me[3], vz: me[4], fx: me[9], fz: me[10], dashCd: me[7], dashT: 0, stunned: 0, input: { x: 0, y: 0, dash: false } };
    ...
    if (me[7] > pr.dashCd + 0.5) pr.dashCd = me[7];
    return pr;
  }
}
